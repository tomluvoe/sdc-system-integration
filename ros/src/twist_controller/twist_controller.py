...
def local_coordinates(pose, waypoints):
...
def initial_cte(pose, waypoints):
    ...
    x_localize, y_localize = localize_coords(pose, waypoints)
    
    #Coefficients (C)
    C = np.polyfit(x_local, y_local, 3)
...
class Controller(object):
    ...
    def control(self, twist_cmd, c_v, t_delta, pose, waypoints):
        ...
        cte_delta = (cte - self.last_cte) / delta_t
        self.last_cte = cte
        ...
        velocity_error = vel_linear - c_v.linear.x

        # Steering prediction
        steer = self.yaw_controller.get_steering(vel_linear, accel_angular, c_v.linear.x)
        

        # Steer correction
        steer_correction = self.pid_steer.step(cte, delta_t, steer, self.S.max_steer_angle)
        steer = steer_correction + steer

        # Throttle/brake prediction
        # Acceleration (a)
        a = velocity_error / delta_t
