...
MARGIN_TO_LIGHT = 3.0
SET_TARGET_SPEED = 20
TARGET_SPEED = SET_TARGET_SPEED/2
...
class WaypointUpdater(object):
    ...
    def publish_waypoints(self):
        ...
        if self.base_waypoints and self.current_pose:
            ...
            if angle > math.pi/4:
                closest_wp += 1
            #rospy.loginfo("wp_updater: Next wp for car x:%f, y:%f is cl_wp:%i redlgt:%d ", car_p.position.x, car_p.position.y, closest_wp, self.upcoming_red_light)
	    ...
	    dist = BRAKE_DISTANCE 
		    ...
		    if distance_to_stop == 0:
            	    ...
            	    #rospy.loginfo("aaawp_updater: car Decel: %f  dist:%i brakingdisy:%i url:%i", deceleration, dist, BRAKE_DISTANCE, self.upcoming_red_light) 
            ...
            #if self.upcoming_red_light >= 0 and closest_wp == self.upcoming_red_light:
            if self.upcoming_red_light >= 0 and dist < BRAKE_DISTANCE:
	        for i in range(closest_wp, self.upcoming_red_light):
	            delta = self.distance(self.base_waypoints, closest_wp, i)
		    rospy.loginfo("aaawp_updater delta Seep :%f %f %f", delta, deceleration, max(TARGET_SPEED-(deceleration*delta),0))
